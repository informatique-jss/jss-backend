...
public class OFXParser {
    ...
    public OFXStatement parseOfx(InputStream file) throws OsirisException {
        try {
            ...
            if (bankRoot != null) {
                ...
                for (int i = 0; i < statementResponseNodes.getLength(); i++) {
                    ...
                    for (int j = 0; j < transactionResponseList.getLength(); j++) {
                        ...
                        var bankTransactionListNode = (Node) bankTransactionListNodeExpr
    ...
    private BankTransactionList parseBankTransactionList(Node bankTransactionListNode) throws Exception {
        ...
        return new BankTransactionList(
                LocalDate.parse(dateStartString, OFX_DATE_FORMAT),
                LocalDate.parse(dateEndString, OFX_DATE_FORMAT),
                trList);
    }
    ...
    private StatementTransaction parseStatementTransaction(Element transaction) {
        ...
        for (int n = 0; n < children.getLength(); n++) {
            ...
            switch (field.getTagName()) {
                case "FITID" -> builder.id(field.getTextContent());
                case "TRNTYPE" ->
                    builder.type(field.getTextContent().equals("null") ? "CREDIT" : field.getTextContent());
                case "NAME" -> builder.name(field.getTextContent());
                case "MEMO" -> builder.memo(field.getTextContent());
                case "DTPOSTED" -> builder.datePosted(LocalDate.parse(field.getTextContent(), OFX_DATE_FORMAT));
                case "DTAVAIL" -> builder.dateAvailable(LocalDate.parse(field.getTextContent(), OFX_DATE_FORMAT));
                case "DTUSER" -> builder.dateUser(LocalDate.parse(field.getTextContent(), OFX_DATE_FORMAT));
                case "TRNAMT" -> builder.amount(field.getTextContent().replaceAll(",", "."));
