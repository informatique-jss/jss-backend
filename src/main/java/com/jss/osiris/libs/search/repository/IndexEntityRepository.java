...
public interface IndexEntityRepository extends QueryCacheCrudRepository<IndexEntity, Integer> {
        ...
        @Query(nativeQuery = true, value = "select entity_id, entity_type,text, search_text from (SELECT index_entity.* , ts_rank_cd(ts_text, websearch_to_tsquery(:searchQuery)) AS rank FROM index_entity WHERE websearch_to_tsquery(:searchQuery)  @@ ts_text) t ORDER BY (case when entity_type in ('Tiers', 'Responsable') then 0.1 else 0 end) + rank desc LIMIT :numberOfResult")
        List<IndexEntity> searchForEntities(@Param("searchQuery") String searchQuery,
                        @Param("numberOfResult") Integer numberOfResult);

        @Query(nativeQuery = true, value = "select entity_id, entity_type, text, search_text from (SELECT index_entity.* , similarity(search_text, :searchQuery)  AS rank FROM index_entity ) t where search_text ILIKE '%'||  :searchQuery ||'%' ORDER BY (case when entity_type in ('Tiers', 'Responsable') then 0.1 else 0 end) + rank desc LIMIT :numberOfResult")
        List<IndexEntity> searchForContainsSimilarEntities(@Param("searchQuery") String searchQuery,
                        @Param("numberOfResult") Integer numberOfResult);

        @Query(nativeQuery = true, value = "select entity_id, entity_type, text, search_text from (SELECT index_entity.* , similarity(search_text, :searchQuery)  AS rank FROM index_entity ) t ORDER BY (case when entity_type in ('Tiers', 'Responsable') then 0.1 else 0 end) + rank desc LIMIT :numberOfResult")
        ...
        @Query(nativeQuery = true, value = "select entity_id, entity_type, text, search_text from (SELECT index_entity.* , similarity(text, :searchQuery)  AS rank FROM index_entity ) t where  entity_type=:entityType and text ILIKE '%'||  :searchQuery ||'%' ORDER BY (case when entity_type in ('Tiers', 'Responsable') then 0.1 else 0 end) + rank desc LIMIT :numberOfResult")
        List<IndexEntity> searchForContainsSimilarEntities(@Param("searchQuery") String searchQuery,
                        @Param("entityType") String entityType,
                        @Param("numberOfResult") Integer numberOfResult);

        @Query(nativeQuery = true, value = "select entity_id, entity_type, text, search_text from (SELECT index_entity.* , similarity(search_text, :searchQuery)  AS rank FROM index_entity where entity_type=:entityType  ) t ORDER BY (case when entity_type in ('Tiers', 'Responsable') then 0.1 else 0 end) + rank desc LIMIT :numberOfResult")
        List<IndexEntity> searchForDeepSimilarEntities(@Param("searchQuery") String searchQuery,
                        @Param("entityType") String entityType,
                        @Param("numberOfResult") Integer numberOfResult);
