...
public class PrintDelegate {
  ...
  public void printMailingLabel(InvoiceLabelResult label, CustomerOrder customerOrder, boolean printProvisionRegister)
      throws OsirisException, OsirisClientMessageException, IOException {
    ...
    try {
      ...
      if (printProvisionRegister) {
        ...
        List<RneCompany> rneCompany = rneDelegateService.getCompanyBySiren(affaire.getSiren());

        String legalFormNumber = (rneCompany != null &&
            rneCompany.get(0).getFormality() != null)
                ? rneCompany.get(0).getFormality().getFormeJuridique().getCode()
                : "";
        ...
        if (affaire.getAddress() != null) {
          ...
          if (affaire.getCedexComplement() != null)
            affaireAddress += " " + affaire.getCedexComplement();
        ...
        if (rneCompany != null &&
            ...
            rneCompany.get(0).getFormality().getContent().getPersonneMorale().getAdresseEntreprise()
                .getAdresse() != null) {
          cityRne = rneCompany.get(0).getFormality().getContent().getPersonneMorale().getAdresseEntreprise()
              .getAdresse()
              .getCommune();
        }
        ...
        List<CompetentAuthority> ca = (competentAuthorityType != null)
            ? competentAuthorityRepository.findByCityAndCompetentAuthorityType(city.get(0), competentAuthorityType)
            : null;
        ...
        dOut.writeUTF(
            "            " + StringUtils.stripAccents((legalFormLabel != null ? legalFormLabel : "")).toUpperCase());
