...
public interface PaymentRepository extends CrudRepository<Payment, Integer> {
    ...
    @Query("select i from Payment i where (:isHideAssociatedPayments=false OR (i.invoice is null and i.customerOrder is null)) and i.paymentWay IN :paymentWays and i.paymentDate>=:startDate and i.paymentDate<=:endDate and (:minAmount is null or paymentAmount>=CAST(CAST(:minAmount as string) as double)) and (:maxAmount is null or paymentAmount<=CAST(CAST(:maxAmount as string) as double)) and (:label is null or label like '%' || cast(:label as string) || '%')")
