...
public interface AccountingRecordRepository extends QueryCacheCrudRepository<AccountingRecord, Integer> {
        ...
        @Query(nativeQuery = true, value = "" +
                        " with majoration as ( " +
                        " select i.id, max(coalesce(suspicious_markup,0)) as suspicious_markup,coalesce(sum(p.payment_amount),0) as paymentAmount, "
                        +
                        ...
                        +
                        " i.id as idInvoice, sum(coalesce(ii.pre_tax_price, 0)-coalesce(ii.discount_amount,0))-max(m.paymentAmount) as htAmount, "
                        +
                        "  dueDaysNumber, " +
                        ...
                        " from (select i.*, cast(:accountingDate as date) - due_date  as dueDaysNumber from  invoice i ) i  "
                        +
                        " join invoice_item ii on ii.id_invoice  = i.id " +
                        ...
                        " join tiers t on t.id = r.id_tiers " +
                        " where i.id_invoice_status  = :idInvoiceSend " +
                        " and due_date <(cast(:accountingDate as date) -INTERVAL '180 day')  " +
                        " group by t.id, i.id,i.dueDaysNumber, m.suspicious_markup, m.affaire, " +
                        " coalesce(coalesce(t.denomination, concat(t.firstname, ' ', t.lastname))) ) " +
                        " select  idTiers, tiers,id_commercial,sum(htAmount) as htAmount, " +
                        " round(sum(htAmount*(appliableRate+suspiciousMarkup )),2) as finalAmount, " +
                        " count(*) as nbrInvoice, " +
                        "  affaire, idInvoice, appliableRate,suspiciousMarkup ,dueDaysNumber" +
                        " from t " +
                        " group by idTiers, tiers,id_commercial, affaire, idInvoice,appliableRate,suspiciousMarkup ,dueDaysNumber                      "
                        +
                        " ")
