...
import { Pagination } from '../modules/miscellaneous/model/Pagination';
...
export abstract class AppRestService<T> {
  ...
  getPagedList(params: HttpParams, api: string, successfulMessage: string = "", errorMessage: string = ""): Observable<Pagination<T>> {
    let context: HttpContext = new HttpContext();
    context.set(this.successfulToken, successfulMessage).set(this.errorToken, errorMessage);
    return this._http.get(AppRestService.serverUrl + this.entryPoint + "/" + api, { params, context }) as Observable<Pagination<T>>;
  }
