...
export class AddAccountingRecordComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.displayedColumns.push({ id: "accountingAccountNumber", fieldName: "accountingAccountNumber", label: "N° de compte", valueFonction: (element: any, elements: any[], column: SortTableColumn, columns: SortTableColumn[]) => { if (element && column && element.accountingAccount) return element.accountingAccount.accountingAccountNumber + "-" + element.accountingAccount.accountingAccountSubNumber; return "" } } as SortTableColumn);
  ...
  addAccountingRecord() {
    if (this.accountingRecordForm.valid) {
      ...
      this.appService.displaySnackBar("Veuilliez remplir l'ensemble des champs avant d'ajouter une nouvelle opération", true, 60);
    }
  }
  ...
  saveOperations() {
    if (this.accountingRecordForm.valid && this.accountingRecords && this.accountingRecords.length > 0) {
      if (this.getTotalBalance() == 0) {
        ...
        this.appService.displaySnackBar("Veuilliez à saisir un ensemble d'opérations équilibrées", true, 60);
      }
    } else {
      this.appService.displaySnackBar("Veuilliez saisir au moins une opération valide", true, 60);
    }
  }
}
