...
export const ATTACHMENT_TYPE_ATTACHMENT_TYPE: EntityType = { entityType: 'AttachmentType', tabName: 'Type de pièce-jointe', entryPoint: 'administration/attachment-type' };
  ...
  getProvisionLabel(entity: any) {
    ...
    if (entity.text.services && entity.text.customerOrder) {
      for (let service of entity.text.services) {
        if (service.serviceLabelToDisplay)
          out.push(service.serviceLabelToDisplay);
      ...
      return out.join(" / ") + " / Commande " + entity.text.customerOrder.id + " - " + ((entity.text.customerOrder.customerOrderStatus) ? entity.text.customerOrder.customerOrderStatus.label : "");
    }
    return "";
  }
