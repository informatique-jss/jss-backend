...
export class PremiumHubComponent extends GenericHubComponent<{ id: number }> implements OnInit {
  ...
  override getAllPostByEntityType(selectedEntityType: Post, page: number, pageSize: number, searchText: string, isDisplayNewPosts: boolean): Observable<PagedContent<Post>> {
    return this.postService.getAllPremiumPosts(page, pageSize, searchText, isDisplayNewPosts);
  }

  override getAllTagByEntityType(selectedEntityType: Post, isDisplayNewPosts: boolean): Observable<Array<Tag>> {
    return this.tagService.getAllTagsByPremiumPosts();
  }

  override getMostSeenPostByEntityType(selectedEntityType: Post, page: number, pageSize: number): Observable<PagedContent<Post>> {
    return this.postService.getMostSeenPremiumPost(page, pageSize);
  }
}
